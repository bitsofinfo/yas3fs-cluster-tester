...
public class Yas3fsMountTest {
	...
	public static void main(String[] args) throws Exception {
		...
		try {
			...
			boolean rereadPhaseEnabled = false;
			long rereadRunDurationMS = 0;
			String rereadMode = null;
			...
			if (confPath != null) {
				...
				rereadPhaseEnabled = Boolean.valueOf(props.getProperty("reread.phase.enabled"));
				rereadRunDurationMS = Long.valueOf(props.getProperty("reread.run.duration.ms"));
				rereadMode = props.getProperty("reread.mode");
				rereadLimitedTotalFiles = Integer.valueOf(props.getProperty("reread.mode.limited.total.files.to.reread"));
				rereadLimitedRereadsPerFile = Integer.valueOf(props.getProperty("reread.mode.limited.total.rereads.per.file"));
			...
			// no props, just collect input
				...
				reportsDir = getUserInput("Local path to a directory " + "to write report files to: (NO TRAILING SLASH): ");
				yas3fsCacheDir = getUserInput("Local path to the yas3fs cache directory root: (NO TRAILING SLASH): ");
				...
				rereadPhaseEnabled = Boolean.valueOf(getUserInput("Enable optional final re-read phase: "));
				if (rereadPhaseEnabled) {
					rereadRunDurationMS = Long.valueOf(getUserInput("Re-read overall run-duration MS: "));
					rereadMode = getUserInput("Re-read mode (all or limited): ");
					if (rereadMode.equalsIgnoreCase("limited")) {
						rereadLimitedTotalFiles = Integer.valueOf(getUserInput("Limited mode: total number of files to randomly read: "));
						rereadLimitedRereadsPerFile = Integer.valueOf(props.getProperty("Limited mode: total number of re-reads per file:"));
					}
				}
			...
			getUserInput("Type anything if S3 bucket is mounted and ready to proceeed @ " + s3MountPath +": (type anything, hit enter)");
			...
			Writer reportFileWriter = null;
			try {
				String rand = UUID.randomUUID().toString().replace("-", "").substring(0,4);
				String reportFileName = reportsDir +"/verify_report_"+rand+".txt";
				File reportFile = new File(reportFileName);
				reportFile.createNewFile();
				reportFileWriter = new BufferedWriter(new FileWriter(reportFile));
				...
				System.out.println("     report file will be written to: "+ reportFileName);
				System.out.println("-------------------------------------------");
				...
				List<String> okFilePaths = new ArrayList<String>();
				...
				for (String fileToVerifyLocally : tocsFiles2VerifyLocally) {
					...
					if (!fileToCheckLocally.exists()) {
						missingFilePaths.add(fileToCheckLocally.getAbsolutePath());
					} else {
						okFilePaths.add(fileToCheckLocally.getAbsolutePath());
					}
				}
				
				// store report
				VerifyReport report = new Yas3fsMountTest.VerifyReport();
				report.totalMissing = missingFilePaths.size();
				report.totalOk = okFilePaths.size();
				report.missingFiles = missingFilePaths;
				report.okFiles = okFilePaths;
				
				// write to json
				reportFileWriter.write(gson.toJson(report));
				
				System.out.println("-------------------------------------------");
				System.out.println("DONE: report file written to: " + reportFile.getAbsolutePath());
				System.out.println("-------------------------------------------");
				
			} catch(Exception e) {
				System.out.println("ERROR: in verify stage, verifying and writing report file: " + e.getMessage());
				e.printStackTrace();
			...
			if (rereadPhaseEnabled) {
				...
				System.out.println("");
				System.out.println("-------------------------------------------");
				System.out.println("Re-read phase enabled: mode: "+rereadMode);
				System.out.println("-------------------------------------------");
				
				
				
				ready = getUserInput("Ready? NOTE this will run an 'rm -rf "+ verifyDir+"/*' " +
						"AND 'rm -rf " + yas3fsCacheDir + "/*' (type y or n): ");
				
				if (!ready.equalsIgnoreCase("y")) {
					return;
				}
				
				if (tocsFiles2VerifyLocally.size() == 0) {
					System.out.println("There are no files to verify! exiting");
					return;
				}
				
				/**
				 * Clean verify dir
				 * Clean yas3fs cache dir
				 */
				execDirPurge(verifyDir);
				execDirPurge(yas3fsCacheDir);
				
				List<String> filesToReread = new ArrayList<String>();
				
				// mode limited
				if (rereadMode.equalsIgnoreCase("limited")) {	
					SecureRandom random = new SecureRandom();
					for (int i=0; i<rereadLimitedTotalFiles; i++) {
						String fileToReread = tocsFiles2VerifyLocally.get(
													random.nextInt(tocsFiles2VerifyLocally.size()));
						filesToReread.add(fileToReread);
					}
					
				// mode all
				} else {
					// if ALL just set equal to the entire set of files
					filesToReread = tocsFiles2VerifyLocally;
				}
				
				
				// determine sleep time
				long sleepTimeMS = (rereadRunDurationMS / (filesToReread.size() * rereadLimitedRereadsPerFile));
				System.out.println("Calculated sleep time MS per file based off of total duration ("+rereadRunDurationMS+"MS) of " + sleepTimeMS + "MS");
				
				for (String fileToReread : filesToReread) {
					
					String copyFrom = s3MountPath +"/" +fileToReread;
					String copyTo = verifyDir +"/" +fileToReread;
					
					// exec copy
					execCopy(copyFrom, copyTo);
					
					File fileToCheckLocally = new File(copyTo);
					if (!fileToCheckLocally.exists()) {
						System.out.println("Post re-read(1) (copy) verification failed of: " + copyTo);
					}
					
					// sleep
					Thread.currentThread().sleep(sleepTimeMS);
					
					// if mode is limited and we re-read again per file
					if (rereadMode.equalsIgnoreCase("limited") && rereadLimitedRereadsPerFile > 0) {
						
						for (int i=0; i<rereadLimitedRereadsPerFile; i++) {
							...
							if (!fileToCheckLocally.exists()) {
								System.out.println("Post re-read("+(i+2)+") (copy) verification failed of: " + copyTo);
							}
	...
	private static void execDirPurge(String directoryPathToPurge) throws Exception {
		
		File toPurge = new File(directoryPathToPurge);
		if (!toPurge.exists()) {
			System.out.println("Path to purge does not exist " + toPurge.getAbsolutePath());
			return;
		}
		
		String command = "rm -rf " + toPurge.getAbsolutePath() + "/*";
		System.out.println("Executing: " + command);
	...
	private static void generateFiles(List<FileInfo> allFiles, 
		...
		if (currDirDepthHolder[0] < maxDirDepth) {
			...
			for (int i=0; i<totalPossibleDirsHere; i++) {
				File dir = new File(rootDirectory + "/dir_" + generateHostId() + "_"+ UUID.randomUUID().toString().replace("-", "").substring(0,6));
				dir.mkdir();
	...
	private static File generateFile(String rootPath, long totalSize) throws Exception {
		File file = new File(rootPath + "/file_" + generateHostId() + "_"+ UUID.randomUUID().toString().replace("-", "").substring(0,8)+ ".txt");
		file.createNewFile();
	...
	public static class VerifyReport {
		...
		public List<String> missingFiles = null;
		public List<String> okFiles = null;
	}
	
	private static String generateHostId() throws Exception {
		String hostId = determineHostName().replaceAll("\\.", "_");
		hostId = hostId.replaceAll(" ", "_");
		return hostId;
	}
}
